...
def compute_loss(points, probs, roi_mask=None):
    ...
    tgt_list = get_tgt_list(points, probs, roi_mask=roi_mask) # will contain all the points to penalise, along with their scales
    # scales means the coefficient
...
def get_tgt_list(points, probs, roi_mask=None):
    ...
    if 0 in u_list:
        ...
        tgt_list += [{'scale': 1, 'ind_list':[ind_bg], 'label':0}]    # label is 0 - meaning: it should not be present

    if 1 in u_list:
        ind_fg = pr_flat.argmax()
        tgt_list += [{'scale': 1, 'ind_list':[ind_fg], 'label':1}]     # label is 1 - meaning: it should be present

    # point level
    if 1 in u_list:
        ind_fg = torch.where(pt_flat==1)[0]
        tgt_list += [{'scale': len(ind_fg), 'ind_list':ind_fg, 'label':1}]  # label is 1 - meaning: it should be present
    ...
    if n_total > 1:
        ...
        for u in fg_uniques:
            ...
            tgt_list += [{'scale': (n_points - 1), 'ind_list':ind_bg, 'label':0}]  # n_points - 1 because every object in the blob is penalised (there are 2 objects == we penalise 2-1 = 1 object)
